...
class FarmView(discord.ui.View):
    ...
    async def criar_farm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if guild.get_role(CARGO_FARM_OK_ID) in member.roles:
            ...
            await interaction.followup.send("🚫 Você já possui uma pasta farm criada.", ephemeral=True)
            return
        ...
        if not isinstance(categoria, discord.CategoryChannel):
            ...
            await interaction.followup.send("❌ Categoria de farm não encontrada ou inválida.", ephemeral=True)
            return
        ...
        try:
            ...
            embed = discord.Embed(
                title="📋 Meta da Farm",
                description="Estas são suas metas de farm diário!\n\n✍️ Edite aqui suas metas diárias.",
            ...
            await interaction.followup.send("✅ Sua pasta foi criada com sucesso!", ephemeral=True)

        except discord.Forbidden:
            print("[FARM] ❌ Permissão insuficiente para criar canal.")
            await interaction.followup.send("❌ Não tenho permissão para criar canal na categoria.", ephemeral=True)
        except Exception as e:
            print(f"[FARM] ❌ Erro inesperado: {e}")
            await interaction.followup.send("❌ Ocorreu um erro ao criar sua pasta farm.", ephemeral=True)
